...
def extract_element(docx_path: str) -> Tuple[list, DocMeta]:
    ...
    doc_meta = DocMeta(
        doc_id=doc_id, title=title, source_type="docx", headers=headers, footers=footers
    )
...
def split_and_chunk(
    ...
    for section in section_leaves:
        ...
        if text_sec:
            ...
            chunk_size, chunk_overlap = pick_chunk_size(
                doc_type=doc_type, style=dominant_style, chunk_type="text"
            )
        ...
        if media_list:
            for _, media in media_list:
                if isinstance(media, ImageElement):
                    ...
                    chunks.append(chunk)
                    image_to_chunks.setdefault((doc_id, media.element_id), []).append(
                        chunk
                    )
                    chunk_index += 1
                    ...
                    if len(rows) < 15:
                        ...
                        for w in range(0, len(data_rows), step):
                            ...
                            prefix = (
                                header_lines
                                if w == 0
                                else f"[TABLE_{media.element_id} continued]"
                            )
                            full_text = f"{prefix}\n" + "\n".join(batch)
                            chunk = Chunk(
...
def process_docx(docx_path: str):
    ...
    Returns:
        chunks          - List[Chunk] ready for ingest + FAISS
