...
def create_transcriptions(audio_path: str) -> Tuple[Transcript, TranscriptMeta]:
    ...
    segments_iter, info = model.transcribe(audio_path, language="en", vad_filter=True)
    ...
    meta = TranscriptMeta(
        doc_id=doc_id, source_type="audio", title=doc_id, len_audio=len_audio
    )
...
def chunk(
    transcript: Transcript, meta: TranscriptMeta
) -> Tuple[List[Chunk], TranscriptMeta]:
    ...
    for seg in transcript.segments:
        timeline_text.append(f"[{seg.start:.2f}s - {seg.end:.2f}s] {seg.text}")
