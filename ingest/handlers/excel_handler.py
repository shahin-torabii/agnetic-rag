...
def effective_value(ws: Worksheet, row: int, col: int, merge_value: Dict) -> Any:
    if (row, col) in merge_value:
        return merge_value[(row, col)]
    return ws.cell(row, col).value


def is_header_row(ws: Worksheet, row_num: int, max_col: int, merge_value: Dict) -> bool:
...
def build_col_paths(
    ws: Worksheet, header_rows: List[int], merge_value: Dict
) -> Dict[int, str]:
...
def row_to_dict(cells: List[ExcelCell], col_paths: Dict[int, str]) -> Dict[str, Any]:
    row_to_dict = {
...
def extract_rows(excel_path: str) -> Tuple[List[Sheet], ExcelMeta]:
    ...
    doc_meta = ExcelMeta(
        doc_id=doc_id, title=title, source_type="xlsx", num_sheet=len(wb_val.sheetnames)
    )
    sheets: List[Sheet] = []
    element_id = 0
    ...
    for sheet_index, sheet_name in enumerate(wb_val.sheetnames):
        ...
        header_cells = [all_rows[r] for r in header_row_nums if r in all_rows]
        data_cells = [
            all_rows[r]
...
def chunk(sheets: List[Sheet], doc_meta: ExcelMeta) -> Tuple[List[Chunk], ExcelMeta]:
    ...
    for sheet in sheets:
        ...
        if len(rows) <= 15:
            row_text = "\n\n".join(build_row_text(r) for r in rows)
            ...
            for start in range(0, len(rows), step):
                ...
                row_text = "\n\n".join(build_row_text(r) for r in rows[start:end])
...
if __name__ == "__main__":
    ...
    for c in chunks:
        print(
            f"[{c.chunk_index:03d}] type={c.chunk_type:<14} tokens={c.token_count:<4} "
        )
