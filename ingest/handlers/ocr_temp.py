...
def _lines_to_elements(text: str, page: int, doc_id: str,
    ...
    for line in text.splitlines():
        ...
        elements.append(TextElement(
            text=line, page=page, style=style,
            section_path=list(stack),
            element_id=counter[0], doc_id=doc_id, source=source,
        ))
        counter[0] += 1
    return elements
...
def extract_pages(pdf_path: str) -> Tuple[List, PdfMeta]:
    ...
    pdf_meta = PdfMeta(
        doc_id=doc_id, title=meta_title,
        num_pages=len(pdf_pdfium), needs_ocr=True,
    )
...
def _classify_pdf(meta: PdfMeta, body_elements: list) -> DocType:
    ...
    headings = " ".join(e.text for e in body_elements
                        if isinstance(e, TextElement)
                        and e.style in HEADING_STYLES).lower()

    scores: Dict[DocType, int] = {dt: 0 for dt in DocType}
    for dt, keywords in DOC_TYPE_SIGNALS.items():
        for kw in keywords:
            if kw in high:     scores[dt] += 2
            if kw in sample:   scores[dt] += 1
            if kw in headings: scores[dt] += 2
        ...
        # ── TEXT ──────────────────────────────────────────────
            ...
            for group in sub_groups:
                ...
                if _token_count(section_text) <= size:
                    chunks.append(Chunk(
                        text=section_text, doc_id=doc_id,
                        chunk_index=chunk_index, chunk_type="text",
                        style=dominant_style, section_path=dominant_path,
                    ...
                    splitter = RecursiveCharacterTextSplitter(
                        chunk_size=get_config().chunking.max_tokens, chunk_overlap=get_config().chunking.overlap_tokens,
                        separators=["[SECTION]", "[SUBSECTION]", "[SUBSUBSECTION]",
                                    "\n\n", "\n", ". ", " ", ""],
                    )
                    for sc in splitter.split_text(section_text):
                        chunks.append(Chunk(
                            text=sc, doc_id=doc_id,
                            chunk_index=chunk_index, chunk_type="text",
                            style=dominant_style, section_path=dominant_path,
                            start_element_id=group[0].element_id,
                            end_element_id=group[-1].element_id,
...
if __name__ == "__main__":
    ...
    for c in text_c[:20]:
        ...
        print(f"  [{c.chunk_index:03d}] tokens={c.token_count:4d}  "
